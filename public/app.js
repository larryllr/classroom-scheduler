...
let state = { tab:"rooms", rooms:[], classes:[] };
...
${state.rooms.map(r=>`${r.name}  容量:${r.capacity}  优先级:${r.priority}`).join("\n") || "（暂无教室）"}
  </pre>
  `;
}
...
async function renderClasses(){
  ...
  <pre style="background:#fff;border:1px solid #ddd;padding:10px;border-radius:8px;margin-top:10px;overflow:auto">
${state.classes.map(c=>`${c.id}. ${c.name}  人数:${c.size}  当天可换教室:${c.allow_switch? "是":"否"}`).join("\n") || "（暂无班级）"}
  </pre>
  `;
}
...
window.addClass = async ()=>{
  ...
  const r = await jpost("/api/classes", { name, size, allow_switch });
  if(r.ok===false) return $("#msg").innerText = r.message;
  $("#msg").innerText="✅ 已新增班级";
  renderClasses();
};
...
async function renderCourses(){
  ...
  <div style="display:grid;gap:6px;max-width:520px">
    ...
    <input id="cweek" type="number" placeholder="周次，如 1（先做第1周）" value="1" />
    <select id="cday">
    ...
    <button onclick="addCourse()">新增课程（第1周）</button>
  </div>
  <div style="margin-top:10px;color:#666">提示：先把第1周课程都录完 → 点“一键生成” → 去“课表复制”。</div>
  `;
}
...
window.solve = async ()=>{
  ...
  $("#msg").innerText = r.ok===false ? ("❌ "+r.message) : "✅ 生成成功！去“课表复制”页复制发群";
};
